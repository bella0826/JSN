...
with torch.no_grad():
    for i, data in enumerate(datasets.testloader):
        ...
        secret_rev = bacward_img.narrow(1, c.channels_in, bacward_img.shape[1] - c.channels_in)
        #secret_rev = iwt(secret_rev)
        #cover_rev = bacward_img.narrow(1, 0, 4 * c.channels_in)
        cover_rev = bacward_img.narrow(1, 0, c.channels_in)
        #cover_rev = iwt(cover_rev)
        resi_cover = (output_steg - cover) * 20
