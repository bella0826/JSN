...
def iwt_init(x):
    #input size = 1, 12, 512, 512
    #output size = 1, 3, 1024, 1024
    r = 2
...
class VGG_conv0_the_same_size(nn.Module):
    def __init__(self, in_nc, nf):

        super(VGG_conv0_the_same_size, self).__init__()

        self.conv0_0 = nn.Conv2d(in_nc, nf, 3, 1, 1, bias=True)
        self.conv0_1 = nn.Conv2d(nf, nf, 3, 1, 1, bias=False)
        self.bn0_1 = nn.BatchNorm2d(nf, affine=True)
        self.conv0_2 = nn.Conv2d(nf, nf, 3, 1, 1, bias=True)
        self.bn0_2 = nn.BatchNorm2d(nf, affine=True)
        self.conv0_3 = nn.Conv2d(nf, nf, 3, 1, 1, bias=True)
        self.bn0_3 = nn.BatchNorm2d(nf, affine=True)
        self.lrelu = nn.LeakyReLU(negative_slope=0.2, inplace=True)

    def forward(self, x):
        fea = self.lrelu(self.conv0_0(x))
        fea = self.lrelu(self.bn0_1(self.conv0_1(fea)))
        fea = self.lrelu(self.bn0_2(self.conv0_2(fea)))
        fea = self.lrelu(self.bn0_3(self.conv0_3(fea)))
        return fea
...
class Res(nn.Module):
    def __init__(self, input, output):
        ...
        mutil.initialize_weights([self.conv4], 0.)

    def convblock1(self, x):
        x = self.lrelu(self.conv0(x))
        x = self.lrelu(self.conv1(x))
        return x
    def convblock2(self, x):
